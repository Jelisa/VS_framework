...
def parse_pele_report(file2parse):
    """
    A function to parse the report file generated by PELE
    :param file2parse: a string containing the name of the file to parse
    :return: two lists on containing the header of the file and the other containing the values
    """
    report_values = []
    with open(file2parse, 'r') as report_file:
        header = []
        first_line = True
        for x, line in enumerate(report_file.readlines()):
            line = line.strip()
            if not line:
                continue
            ...
            if first_line:
                ...
                if len(split_line) != len(header):
                    print "There's an error in the line {0} in the file {1}, so it will be skipped you should check it".format(x+1, file2parse)
                    #print line, split_line
                    report_values.append([0 for x in range(len(header))])
                    continue
                for v in split_line:
                    try:
                        values.append(int(v))
    ...
    return header, report_values


def extract_minimum_energy_models(report_filename, number_skipped_systems, deltag_limit, max_step):
    # This block will analyze the report file
    report_header, report_values = parse_pele_report(report_filename)
    ...
    steps_idx = report_header.index("Step")
    if max_step == 0:
        max_step = report_values_array[-1, steps_idx]
    # print 'c', max_step
    binding_energy_index = report_header.index("Binding Energy")
    minimum_energy_value = report_values_array[report_values_array[:,steps_idx] <= max_step, binding_energy_index].min()
    minimum_energy_model = report_values_array[report_values_array[:,steps_idx] <= max_step,
                                               binding_energy_index].argmin()
...
parser.add_argument("-log_file", default="structures_extraction.log", help=parameters_help.log_file)
    ...
    try:
        models_2_use, pele_minimum_energy_model, pele_energies_values_array,\
            pele_binding_energy_index, pele_minimum_energy_value = extract_minimum_energy_models(pele_report_filename,
                                                                                                 ...
                                                                                                 args.deltag,
                                                                                                 args.max_steps)
        # print pele_minimum_energy_model
    except ValueError:
        print "error, can't extract the minimum energy model"
        continue
